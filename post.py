...
class Post(BaseModel):
    ...
    def dict(self, **kwargs):
        ...
        # Если были переданы дополнительные параметры, применяем их к результату
        if kwargs.get('exclude') and 'raw' in kwargs['exclude']:
            pass  # raw уже исключен
        return result
    ...
    def parse_post(cls, cubus_post):
        try:
            ...
            return cls(**data)  # Используем ** вместо model_validate
